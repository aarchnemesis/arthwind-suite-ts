...
export default function BladeSplitSelector({
  ...
  const updateSession = (bladeIdx, sessIdx, field, value) => {
    if (field === 'novo_blade_position' && value && !VALID_POSITIONS.includes(value.toUpperCase())) return;
    ...
    const newSuspeitos = bladeSplitSuspeitos.map((b, i) => {
      ...
      const newSessoes = b.sessoes.map((s, j) => {
        ...
        return {
          ...s,
          [field]: field === 'novo_blade_position' ? value.toUpperCase() : value,
        };
      });
    ...
    if (isCsv) {
      ...
      const correcoes = newSuspeitos
        .map(b => {
          const sessoesPreenchidas = b.sessoes
            .filter(s => s.novo_blade_position && s.novo_blade_sn && s.novo_blade_sn.trim())
            .map(s => ({
  ...
  return (
    <div className="gps-selector-container" style={{ background: D.logBg, border: `1px solid ${D.borderLight}` }}>
      <div className="gps-selector-header" style={{ color: D.accent, borderBottom: `1px solid ${D.borderLight}` }}>
        {Icons.warn(D.accent)} {isCsv ? 'Sessões de voo detectadas (por Blade SN)' : 'Sessões de voo detectadas (por posição)'}
      </div>
      <div className="gps-list">
        {bladeSplitSuspeitos.map((b, bIdx) => {
          ...
          return (
            <div key={bIdx} className="split-item" style={{
              padding: '12px',
              ...
              <div style={{ color: D.textPrimary }}>
                <strong>{label}</strong>
                <span style={{ color: D.textSecond, marginLeft: 8, fontSize: '0.9em' }}>
              ...
              {(b.sessoes || []).map((s, sIdx) => (
                <div key={sIdx} style={{
                  ...
                  <div style={{ color: D.textSecond, fontSize: '0.85em' }}>
                    <strong style={{ color: D.textPrimary }}>Sessão {s.session_idx + 1}</strong>
                    {' • '}{s.total_fotos} fotos
                    {' • '}{fmtTs(s.ts_inicio)} → {fmtTs(s.ts_fim)}
                    {s.sides && s.sides.length > 0 && ` • sides: ${s.sides.join(', ')}`}
                  </div>
                    ...
                    {!isCsv && (
                      <input
                        ...
                        style={{
                          ...
                          color: D.textPrimary, padding: '6px', borderRadius: '4px', width: '130px'
                        }}
                        value={s.novo_blade_position || ''}
                        onChange={e => updateSession(bIdx, sIdx, 'novo_blade_position', e.target.value)}
                      />
                    )}
                    <input
                      type="text"
                      ...
                      onChange={e => updateSession(bIdx, sIdx, isCsv ? 'novo_sn' : 'novo_blade_sn', e.target.value)}
                    />
                  </div>
                </div>
              ))}
