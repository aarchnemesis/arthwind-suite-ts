...
export default function ArthnexUploaderModule({ T, D }) {
  ...
  const carregarWorkorders = useCallback(async () => {
    if (typeof window.pywebview === 'undefined') return;
    setLoadingWo(true);
    setWorkorders([]); setWorkorderId(''); setWorkorderQuery(''); setPendingBlades([]); setManualOverrides({}); setCsvAnalysis({});
    try {
  ...
  const carregarPas = useCallback(async (woId) => {
    if (!woId || typeof window.pywebview === 'undefined') {
      setPendingBlades([]);
      return;
    }
    setLoadingPas(true);
    try {
      const res = await window.pywebview.api.arthnex_listar_pas(woId, useHomolog);
      if (res.success) {
  ...
  const turbineList = useMemo(() => Object.keys(bladesPorTurbina).sort(), [bladesPorTurbina]);

  useEffect(() => {
    if (turbineList.length > 0 && (!selectedTurbineModal || !bladesPorTurbina[selectedTurbineModal])) {
  ...
  const onUpload = async () => {
    ...
    const res = await window.pywebview.api.arthnex_upload_multi(
      csvPaths, workorderId, pSurface, collectDate, useHomolog, manualOverrides
    );
    setRunning(false);
    setResult(res);
    if (res && res.success && (!res.arquivosComFalha || res.arquivosComFalha === 0)) {
      setCsvPaths([]);
      setManualOverrides({});
      setCsvAnalysis({});
    }
  ...
  const fieldStyle = {
    ...
    border: `1px solid ${D.border}`, background: D.inputBg, color: D.textPrimary, fontSize: '13px',
  };
  const labelStyle = { fontSize: '12px', fontWeight: 600, color: D.textSecond, marginBottom: '5px', display: 'flex', alignItems: 'center', gap: '6px' };

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '18px', padding: '18px', maxWidth: '750px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
        <label style={{ fontSize: '12.5px', color: D.textSecond, display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
      ...
      <div>
        <label style={labelStyle}>
          Workorder {workorderId && !loadingWo ? Icons.checkCircle(D.success) : null} {loadingPas ? '(carregando pás...)' : ''}
        </label>
        <input
          style={fieldStyle} list="arthnex-wo-options"
          value={workorderQuery} disabled={loadingWo || running}
          placeholder={loadingWo ? 'Carregando workorders...' : 'Digite para buscar por nome ou número...'}
      ...
      <div>
        ...
        <input type="date" style={{ ...fieldStyle, maxWidth: '240px' }} value={collectDate} disabled={running}
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
          <label style={{ ...labelStyle, marginBottom: 0 }}>
            CSVs de upload — um por pá. O pareamento por turbina e pá é automático, mas você pode buscar turbinas ou resolver ambiguidades abaixo.
          </label>
        </div>
        ...
        <div style={{ display: 'flex', gap: '10px', marginTop: '8px' }}>
          <button onClick={pickCsvs} disabled={running || !workorderId} style={{
            ...
            padding: '8px 14px', borderRadius: '6px', border: `1px solid ${D.border}`,
            background: D.bgCard, color: D.textPrimary, cursor: (running || !workorderId) ? 'not-allowed' : 'pointer', fontSize: '12.5px', fontWeight: 500
          }}>
          ...
          {pendingBlades.length > 0 && (
            <button onClick={() => setShowModal(true)} disabled={running} style={{
              display: 'flex', alignItems: 'center', gap: '6px',
              padding: '8px 14px', borderRadius: '6px', border: `1px solid ${D.accent}`,
              background: D.accent, color: '#fff', cursor: running ? 'not-allowed' : 'pointer', fontSize: '12.5px', fontWeight: 600,
            }}>
        ...
        {csvPaths.length > 0 && (
          <div style={{
            ...
            border: `1px solid ${D.border}`, borderRadius: '6px', padding: '8px', maxHeight: '300px', overflowY: 'auto', background: D.bgCard,
          }}>
            {csvPaths.map(p => {
              ...
              const filteredEntries = Object.entries(bladesPorTurbina).filter(([turb]) => {
                if (!searchVal.trim()) return true;
                return turb.toLowerCase().includes(searchVal.toLowerCase().trim());
              });

              const matchedBlade = pendingBlades.find(b => String(b.id) === String(selectedBladeId));

              return (
                <div key={p} style={{
                  display: 'flex', flexDirection: 'column', gap: '4px',
                  padding: '8px 10px', background: D.inputBg,
                  borderRadius: '6px', border: `1px solid ${selectedBladeId ? D.accent : D.border}`
                }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '12px', color: D.textSecond }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '280px' }} title={p}>
                      {Icons.fileText(D.accent)}
                      <strong style={{ color: D.textPrimary }}>{filename}</strong>
                    </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <input
                        ...
                        placeholder="🔍 Turbina..."
                        value={searchVal}
                        onChange={(e) => setTurbineSearch(prev => ({ ...prev, [p]: e.target.value }))}
                        disabled={running}
                        style={{
                          padding: '4px 7px', fontSize: '11px', borderRadius: '4px',
                          border: `1px solid ${D.border}`, background: D.bgCard, color: D.textPrimary, width: '110px'
                        }}
                      />
                      ...
                      <select
                        ...
                        disabled={running}
                        style={{
                          padding: '4px 8px', fontSize: '11.5px', borderRadius: '5px',
                          border: `1px solid ${selectedBladeId ? D.accent : D.border}`,
                          background: selectedBladeId ? `${D.accent}18` : D.bgCard,
                          color: selectedBladeId ? D.accent : D.textPrimary, cursor: 'pointer', maxWidth: '220px'
                        }}
                      >
                      ...
                      <button onClick={() => removeCsv(p)} disabled={running} style={{
                        border: 'none', background: 'none', color: D.error, cursor: running ? 'not-allowed' : 'pointer', padding: '2px 4px', display: 'flex', alignItems: 'center'
                      }}>
                        {Icons.close(D.error)}
                      </button>
                    </div>
                  ...
                  {selectedBladeId && (
                    <div style={{ fontSize: '11px', color: D.success, fontWeight: 500, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: `${D.success}10`, padding: '3px 8px', borderRadius: '4px' }}>
                      <span>✓ Vincular a: <strong>{matchedBlade ? `Turbina ${matchedBlade.turbine} — Pá ${matchedBlade.blade}` : `Pá ID ${selectedBladeId}`}</strong></span>
                      <button onClick={() => setOverrideForCsv(p, '')} style={{ border: 'none', background: 'none', color: D.textMuted, cursor: 'pointer', fontSize: '10.5px', textDecoration: 'underline' }}>
                  ...
                  {!selectedBladeId && analysis?.status === 'ambiguous' && (
                    <div style={{ background: '#ff980018', border: '1px solid #ff980066', borderRadius: '5px', padding: '6px 9px', fontSize: '11.5px', color: D.textPrimary }}>
                      <div style={{ fontWeight: 600, color: '#ff9800', marginBottom: '4px' }}>
                        ⚠️ Ambiguidade: Pá '{analysis.bladeRaw}' encontrada em {analysis.candidateBlades?.length} turbinas pendentes nesta Workorder.
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
                        <span style={{ fontSize: '11px', color: D.textSecond }}>Selecione a turbina:</span>
                        {analysis.candidateBlades?.map(cand => (
                          <button
                            ...
                            disabled={running}
                            style={{
                              padding: '2px 8px', borderRadius: '4px', border: `1px solid ${D.accent}`,
                              background: `${D.accent}22`, color: D.accent, fontSize: '11px', fontWeight: 600, cursor: 'pointer'
                            }}
                          >
                  ...
                  {!selectedBladeId && analysis?.status === 'no_match' && (
                    <div style={{ background: '#f4433614', border: '1px solid #f4433644', borderRadius: '5px', padding: '5px 8px', fontSize: '11px', color: D.error }}>
                      ⚠️ Nenhuma turbina auto-detectada para a pá '{analysis.bladeRaw || 'não informada'}'. Digite a turbina no campo ao lado para selecionar.
                    </div>
                  )}
                </div>
              );
      ...
      <button onClick={onUpload} disabled={!canRun} style={{
        ...
        padding: '12px', borderRadius: '6px', border: 'none', cursor: canRun ? 'pointer' : 'not-allowed',
        background: canRun ? D.accent : D.border, color: '#fff', fontSize: '13.5px', fontWeight: 600, opacity: canRun ? 1 : 0.6,
      }}>
        {Icons.upload('#fff')}
        {running
          ? (batchProgress
      ...
      {result && (
        <div style={{
          padding: '12px 14px', borderRadius: '6px', fontSize: '12.5px',
          ...
          {result.success ? (
            <>
              <div style={{ fontWeight: 600, display: 'flex', alignItems: 'center', gap: '6px' }}>
                {Icons.checkCircle(D.success)} Lote concluído: {result.totalEnviados}/{result.totalFotos} fotos enviadas em {result.resultados.length} arquivo(s), {result.arquivosComFalha} com falha.
              </div>
              {result.resultados.map((r, i) => (
                <div key={i} style={{ marginTop: '5px', color: r.success ? D.textSecond : D.error, fontSize: '12px' }}>
                  • {r.arquivo}: {r.success ? `${r.enviados}/${r.total} enviadas — Turbina: ${r.turbine || 'N/A'} | Pá: ${r.blade || 'N/A'}` : `falhou — ${r.error}`}
                </div>
              ))}
            </>
          ) : (
      ...
      {showModal && (
        <div style={{
          ...
          background: 'rgba(0, 0, 0, 0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999, padding: '20px'
        }}>
          <div style={{
            background: D.bgCard, border: `1px solid ${D.border}`, borderRadius: '8px',
            width: '100%', maxWidth: '640px', padding: '20px', display: 'flex', flexDirection: 'column', gap: '14px',
            boxShadow: '0 8px 32px rgba(0,0,0,0.5)', color: D.textPrimary
          }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: `1px solid ${D.border}`, paddingBottom: '10px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <h3 style={{ margin: 0, fontSize: '15px', color: D.textPrimary, fontWeight: 600 }}>Mapeamento de Pás por Turbina</h3>
              </div>
              <button onClick={() => setShowModal(false)} style={{ border: 'none', background: 'none', color: D.textSecond, cursor: 'pointer', padding: '4px' }}>
            ...
            <div>
              ...
              >
                {turbineList.map(t => (
                  <option key={t} value={t}>Turbina: {t} ({bladesPorTurbina[t]?.length || 0} pás pendentes)</option>
                ))}
              </select>
            </div>

            {selectedTurbineModal && bladesPorTurbina[selectedTurbineModal] && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', background: D.inputBg, padding: '12px', borderRadius: '6px', border: `1px solid ${D.border}` }}>
                <div style={{ fontSize: '12px', color: D.textSecond, fontWeight: 600 }}>
                  Pás pendentes da Turbina {selectedTurbineModal}:
                </div>

                {bladesPorTurbina[selectedTurbineModal].map(bladeItem => {
                  const assignedCsv = csvPaths.find(p => String(manualOverrides[p]) === String(bladeItem.id)) || '';

                  return (
                    <div key={bladeItem.id} style={{
                      display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                      padding: '8px 10px', background: D.bgCard, borderRadius: '5px', border: `1px solid ${D.border}`
                    }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                        {Icons.leaf(D.accent)}
                        <div>
                          <div style={{ fontSize: '12.5px', fontWeight: 600, color: D.textPrimary }}>
                            Pá: {bladeItem.blade}
                          </div>
                          <div style={{ fontSize: '11px', color: D.textMuted }}>ID Arthnex: {bladeItem.id}</div>
                        </div>
                      </div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <select
                          ...
                          style={{
                            padding: '5px 9px', fontSize: '12px', borderRadius: '5px',
                            border: `1px solid ${assignedCsv ? D.accent : D.border}`,
                            background: assignedCsv ? `${D.accent}18` : D.inputBg,
                            color: assignedCsv ? D.accent : D.textPrimary, cursor: 'pointer', maxWidth: '280px'
                          }}
                        >
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '10px' }}>
              <button
                ...
                style={{
                  ...
                  background: D.accent, color: '#fff', fontSize: '12.5px', fontWeight: 600, cursor: 'pointer'
                }}
              >
                Concluído
              </button>
