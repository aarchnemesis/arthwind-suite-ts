...
export default function GoproRawSelector({ regioes, setOptions, D }) {
  const [cfg, setCfg] = useState(() =>
    Object.fromEntries((regioes || []).map(r => [
      r.region,
  ...
  const inputStyle = {
    ...
    color: D.textPrimary, padding: '5px 7px',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '6px' }}>
      {regioes.map(({ region, num_min, num_max, total }) => {
        const c = cfg[region] || { z_inicial: '', paired: false, transition: '', photocard: '' };
        ...
        return (
          <div key={region} style={{
            background: D.logBg, border: `1px solid ${D.borderLight}`,
            borderRadius: '6px', padding: '10px 12px',
          }}>
            {/* Header */}
            <div style={{ marginBottom: '10px' }}>
              <strong style={{ color: D.accent, fontSize: '13px' }}>{region}</strong>
              <span style={{ color: D.textSecond, fontSize: '11px', marginLeft: '8px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '10px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <span style={{ fontSize: '12px', color: D.textSecond, whiteSpace: 'nowrap' }}>
                  Z inicial (mm):
                </span>
                <input
                  type="number"
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <label style={{
                ...
                cursor: 'pointer', fontSize: '12px', color: D.textSecond,
              }}>
                <input
                  type="checkbox"
                  checked={c.paired}
              ...
              {c.paired && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                  <span style={{ fontSize: '11px', color: D.textMuted }}>até GOPR</span>
                  <input
                    type="number"
                  ...
                  />
                  <span style={{ fontSize: '11px', color: D.textMuted }}>(vazio = todos)</span>
                </div>
              )}
            </div>
            ...
            {/* Preview */}
            <div style={{
              background: D.bgDeep, borderRadius: '4px', padding: '6px 10px',
              fontSize: '11px', color: D.textSecond, marginTop: '10px',
              display: 'flex', gap: '16px', flexWrap: 'wrap',
            }}>
              <span>posições: <strong style={{ color: D.textPrimary }}>{nPos}</strong></span>
              <span>passo: <strong style={{ color: D.textPrimary }}>500 mm</strong></span>
              <span>Z: <strong style={{ color: D.success }}>{zIni} → {zMax} mm</strong></span>
              {c.paired && <span style={{ color: D.warning }}>· pares ativos</span>}
            </div>
          </div>
        );
      })}
