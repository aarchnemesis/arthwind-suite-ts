...
export default function usePyWebView() {
  ...
  const handleRun = useCallback((active, filePaths, options, gpsRaiz) => {
    if (isPyWebView) {
      ...
      else if (active === 11) api.reconstruir_csv(fotosDir, options.blade_sn || '', options.sides || []);
      else if (active === 12) api.packer_plataforma(bladeDir);
      else if (active === 13) api.arthnex_organizar(fotosDir, dryRun);
      else if (active === 15) api.calibrar_z_gopro_raw(
          bladeDir,
          options.turbine  || '',
          options.blade_sn || '',
          options.referencias || {},
      ...
      else if (active === 21) api.enviar_videos_drive(localTurbineDir, driveTurbineDir, dryRun);
      return;
    }
  ...
  const carregarFotosReconstruir = useCallback((pasta) => {
    if (!pasta) return;
  ...
  const carregarGoproRaw = useCallback((pasta) => {
    if (!pasta) return;
